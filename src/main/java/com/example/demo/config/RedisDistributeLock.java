...
public class RedisDistributeLock {
     ...
     */
    private static final String DISTRIBUTE_LOCK_SCRIPT_UNLOCK_VAL = "";
    ...
    /**
     * 根据loopTryTime 循环重试 加锁
     *
     * @param lockKey     锁key
     * @param lockVal     锁值，用于解锁校验
     * @param expiryTime  锁过期时间 单位ms
     * @param loopTryTime 获取失败时，循环重试获取锁的时长 单位ms
     * @return 是否获得锁
     */
    ...
    //    //这里也可以我们使用lua脚本实现
    //   return redisService.setnx(lockKey, lockVal, expiryTime);
    //}
    //
    ///**
    ...
    //    argv.add(lockVal);
    //    try {
    //        Object result = redisService.evalsha(unlockSha1, keys, argv);
    ...
    //    if (!StringUtils.hasText(unlockSha1) || !jedisCluster.scriptExists(unlockSha1, slotKey)) {
